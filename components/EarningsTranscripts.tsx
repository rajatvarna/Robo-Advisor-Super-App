...
const EarningsTranscripts: React.FC<EarningsTranscriptsProps> = ({ transcriptsData }) => {
  if (transcriptsData === null) {
    return (
      <div className="flex flex-col items-center justify-center h-96 bg-brand-secondary rounded-lg border border-brand-border">
         <Spinner />
         <p className="mt-4 text-brand-text-secondary">Searching for latest earnings transcripts with AI...</p>
      </div>
    );
  }
  ...
  const { transcripts, sources } = transcriptsData;

  return (
    <div className="bg-brand-secondary border border-brand-border rounded-lg shadow-lg">
      <div className="p-4 border-b border-brand-border">
        <h3 className="text-lg font-bold text-brand-text">Recent Earnings Call Transcripts (AI Generated)</h3>
      </div>
        ...
        {transcripts.map((item) => (
          <details key={item.quarter} className="bg-brand-primary p-4 rounded-lg border border-brand-border group">
            ...
            <div className="mt-4 pt-4 border-t border-brand-border">
              <h4 className="font-bold text-brand-text mb-2">Summary</h4>
              <p className="text-sm text-brand-text-secondary mb-4">{item.summary}</p>
              <h4 className="font-bold text-brand-text mb-2">Key Quote</h4>
      ...
      {sources && sources.length > 0 && (
        <div className="p-4 border-t border-brand-border">
          <h4 className="text-sm font-bold text-brand-text mb-2">Sources</h4>
          <ul className="list-disc list-inside space-y-1">
            {sources.map((source, index) => (
              <li key={index} className="text-xs">
                <a href={source.uri} target="_blank" rel="noopener noreferrer" className="text-brand-accent hover:underline truncate">
